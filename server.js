...
//============ done =======================//
const menuSort = async () => {
 
 
  const menu = await MenuMaster
    .find({ parent: null })
    .lean({ virtuals: true }); // 👈 IMPORTANT
...
function flattenMenu(menuList, prefix = "") {
  let result = [];
...
app.get("/services/:parentSlug/:childSlug", async (req, res) => {
  const { parentSlug, childSlug } = req.params;

  let pageData = await servicePage.findOne({
    slug: childSlug,
...
app.get("/services/*", async (req, res) => {
  const slugPath = req.params[0];        // e.g. "gst/registration/basic"
  const parts = slugPath.split("/");     // [ 'gst', 'registration', 'basic' ]

  // Last part is the slug of the page
  const childSlug = parts[parts.length - 1];

  // Optional: parent path if needed
  const parentSlug = parts.length > 1 ? parts[parts.length - 2] : null;

  // If you store full hierarchy, you can match it as needed
  let pageData = await servicePage.findOne({
      slug: childSlug
  });
...
app.post("/callback", async function (req, res) {
  try {
    ...
    const { full_name, email, mobile,slug } = data;
...
// service page
app.get("/admin/servicePage", async (req, res) =>  {
  let auth = req.isAuthenticated();
  let singleService;
  let menu=await menuSort();
